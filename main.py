#!/usr/bin/env python3
"""
Quantum Income Engine - Single-file fixed version with resilient polling loop
...
async def _run_polling_with_restart(app, stop_event=None):
    backoff = 1
    while True:
        try:
            print("[bot] Starting polling (resilient loop)...")
            app.run_polling(allowed_updates=Update.ALL_TYPES)
            print("[bot] run_polling exited cleanly")
            break
        except Exception as exc:
            msg = str(exc)
            print(f"[bot] Polling crashed: {msg}")
            traceback.print_exc()
            if "Name or service not known" in msg or "Failed to resolve" in msg:
                print("[bot] DNS resolution error detected. Check network/DNS settings.")
            if "401" in msg or "Unauthorized" in msg:
                print("[bot] Authorization error detected. Verify TELEGRAM_TOKEN.")
            if "403" in msg:
                print("[bot] Forbidden: token may be blocked or lack permissions.")
            wait = min(60, backoff) + random.uniform(0, 2)
            print(f"[bot] Restarting polling in {wait:.1f}s...")
            time.sleep(wait)
            backoff = backoff * 2 if backoff < 60 else 60
        if stop_event and stop_event.is_set():
            print("[bot] stop_event set — exiting polling loop")
            break
...
def start_worker_and_bot():
    ...
    t = threading.Thread(target=lambda: asyncio.run(_run_polling_with_restart(app)), daemon=True)
    t.start()
    t.join()
