#!/usr/bin/env python3
"""
Quantum Income Engine - Single-file fixed version

Environment variables (set in Choreo or your host):
TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, TELEGRAM_ADMIN_ID, ADMIN_TOKEN,
ZEALY_COMMUNITIES, GALXE_API_URLS, TASKON_API_URLS, BITGET_EARN_URLS,
BITGET_API_KEY_RW, BITGET_API_SECRET_RW, BITGET_API_PASSPHRASE_RW,
SCAN_INTERVAL_MINUTES (default 15), MIN_TASK_USD (default 20),
PORT (default 8080), DB_PATH (optional)
...
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")  # can be channel or chat id
...
# Aggregator envs (comma separated)
...
API_HOST_BITGET = os.getenv("API_HOST_BITGET", "https://api.bitget.com")
...
# -------------------------
# Telegram send helpers (HTTP requests - reliable)
# -------------------------
def send_text(chat_id, text):
    if not TELEGRAM_TOKEN:
        print("[send_text] no token")
        return
    try:
        ...
        if hasattr(e, 'response') and e.response is not None:
            print(f"[send_text] resp: {e.response.text}")
...
def send_approval_card_sync(aid, payload):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        ...
        return
    try:
        summary_keys = ("type","name","token_symbol","token_amount","est_usd","amount_usd","platform","task_id","title")
        summary = {k: payload.get(k) for k in summary_keys if payload.get(k) is not None}
        ...
        # Telegram InlineKeyboardMarkup object is normally constructed via telegram objects,
        # but when sending directly via HTTP we send the reply_markup as JSON.
        reply_markup = {
            "inline_keyboard": [
                [
                    {"text":"✅ Approve", "callback_data": f"APPROVE::{aid}"},
        ...
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
        payload_data = {'chat_id': str(TELEGRAM_CHAT_ID), 'text': text, 'parse_mode': 'HTML', 'reply_markup': reply_markup}
...
def _urls_from_env(name):
    ...
    return [u.strip() for u in raw.split(",") if u.strip() and _is_valid_url(u.strip())]
...
def _safe_get_json(url, headers=None, params=None, timeout=10):
    try:
        ...
        print(f"[safe_get] SSL error for {url}: {e}")
    except requests.exceptions.ConnectionError as e:
        # Distinguish DNS resolution vs connection refused
        msg = str(e)
        if "Name or service not known" in msg or "Failed to establish a new connection" in msg or "Failed to resolve" in msg:
            print(f"[safe_get] Name resolution/connection error for {url}: {e}")
        else:
            print(f"[safe_get] Connection error for {url}: {e}")
        ...
        try:
            ...
            pass
        print(f"[safe_get] HTTP error {status} for {url}: {e}")
    except requests.exceptions.RequestException as e:
        print(f"[safe_get] request error for {url}: {e}")
    except Exception as e:
        print(f"[safe_get] unexpected error for {url}: {e}")
    return None
...
def scan_zealy():
    ...
    for sub in subs:
        ...
        if not js:
            # If 401 or other, _safe_get_json already logged the error
            continue
        ...
        if isinstance(cand, list):
            for c in cand:
                try:
                    ...
                    continue
    # TaskOn endpoints (but skip invalid/non-resolving hosts)
    for url in _urls_from_env("TASKON_API_URLS"):
        # If host appears to be an unknown/deprecated host, skip automatically
        parsed = urlparse(url)
        host = parsed.netloc
        # Example of skipping specific broken host name; you can add more here
        if host and ("project.taskon.xyz" in host or "taskon.xyz" in host and "project" in host):
            print(f"[scanner] Skipping known-broken host: {host}")
            continue
        js = _safe_get_json(url)
        if not js:
            continue
        ...
        if isinstance(cand, list):
            for t in cand:
                try:
                    ...
                    continue
    # Bitget Earn endpoints (non-auth fallback)
    for url in _urls_from_env("BITGET_EARN_URLS"):
        # Prefer authenticated Bitget call; if URL is a public endpoint, try it
        js = _safe_get_json(url)
        if not js:
            continue
...
def scan_airdrops():
    return scan_generic_aggregators()
...
# -------------------------
# Bitget authenticated requests (for RW actions)
# -------------------------
def _bitget_sign(timestamp, method, request_path, body, secret):
    what = str(timestamp) + method.upper() + request_path + (body or "")
    return hmac.new(secret.encode(), what.encode(), hashlib.sha256).hexdigest()
...
def bitget_request_rw(method, path, params_or_body=None):
    ...
    url = API_HOST_BITGET.rstrip("/") + path
...
# -------------------------
# Core scanning & queueing
# -------------------------
def process_opportunities():
    try:
        print("[scanner] Starting scan...", datetime.utcnow().isoformat())
        found = []
        found += scan_airdrops()
        ...
        for op in found:
            ...
            print(f"[scanner] queued {aid} -> {op.get('id') or op.get('title')}")
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != TELEGRAM_ADMIN_ID:
        ...
        send_text(TELEGRAM_CHAT_ID, f"⚠️ Unauthorized attempt by @{from_user.username} (id:{user_id}) on {cmd} {aid}")
        return
