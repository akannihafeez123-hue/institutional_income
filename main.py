#!/usr/bin/env python3
"""
Quantum Income Engine - Complete Single-File Bot (FIXED)
...
# -------------------------
# Telegram Sending Functions (FIXED - Using Direct HTTP)
# -------------------------
APPLICATION = None  # will be set to Application instance

def send_text(chat_id, text):
    """Send message via direct HTTP call - most reliable"""
    ...
    try:
        ...
        print(f"[send_text] success: {text[:50]}...")
...
def send_approval_card_sync(aid, payload):
    """Send approval card via direct HTTP - most reliable"""
...
def process_opportunities():
    """Main scanner function - runs periodically"""
    try:
        ...
        traceback.print_exc()
        send_text(TELEGRAM_CHAT_ID, f"⚠️ Scanner error: {e}")
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if cmd == "SELL":
            ...
            if not token:
                await query.edit_message_text("⚠️ Sell failed: token info missing.")
                send_text(TELEGRAM_CHAT_ID, "⚠️ Sell failed: no token symbol found.")
                return
...
if __name__ == "__main__":
    ...
    # Run Flask in background thread (for web health checks)
    ...
    try:
        # Start bot and scheduler together (blocking)
        ...
        traceback.print_exc()
        # Attempt notifying admin via direct HTTP call
        try:
