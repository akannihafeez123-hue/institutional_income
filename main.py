#!/usr/bin/env python3
"""
Quantum Income Engine - Complete Single-File Bot (SIGNAL-SAFE)
...
# -------------------------
# Telegram Sending Functions (HTTP-based)
# -------------------------
...
def start_worker_and_bot():
    ...
    # Start bot polling (signal-safe in non-main-thread / restricted envs)
    print("[bot] Starting polling (signal handling disabled)...")
    # NOTE: stop_signals=None prevents the library from registering signal handlers,
    # which avoids the "set_wakeup_fd only works in main thread" runtime error in restricted environments.
