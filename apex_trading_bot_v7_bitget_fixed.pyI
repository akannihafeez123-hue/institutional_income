...
# ═══════════════════════════════════════════════════════════════════════════
# BITGET API CLIENT - UPDATED FOR V3 API WITH CORRECT TIMEFRAMES
# ═══════════════════════════════════════════════════════════════════════════

class BitgetAPI:
    """Production Bitget API client with V3 API endpoints and correct parameters"""
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 200) -> List:
        ...
        # Map interval to Bitget V3 format - CORRECT TIMEFRAMES FROM ERROR MESSAGE
        interval_map = {
            '1min': '1min',
            '3min': '3min',
            '5m': '5min',
        ...
        bitget_interval = interval_map.get(interval.lower())
        if not bitget_interval:
            # Try to find closest match
            for key in interval_map:
                if interval.lower() in key:
                    bitget_interval = interval_map[key]
                    break
        
        if not bitget_interval:
            logging.error(f"Unsupported interval: {interval}. Valid intervals: {list(interval_map.keys())}")
            return []
        ...
        try:
            ...
            async with self.session.get(
                ...
                if data.get('code') == '00000':  # Bitget success code
                    ...
                    logging.error(f"Bitget Kline API error: {data.get('msg', 'Unknown error')} - Data: {data}")
                    return []
        except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════════
# MULTI-TIMEFRAME ANALYZER - UPDATED WITH CORRECT TIMEFRAME MAPPING
# ═══════════════════════════════════════════════════════════════════════════
...
class MultiTimeframeAnalyzer:
    ...
    def __init__(self):
        ...
        # Map our timeframes to Bitget's valid timeframes
        self.timeframe_mapping = {
            ...
            '1h': '1h',
            '4h': '4h',
            '8h': '8h',  # Bitget doesn't have 8h, will use 6h instead
            '12h': '12h',
            '1d': '1day',
            '1w': '1week',
            '1M': '1M'
    ...
    async def analyze_all(self, client: BitgetAPI, symbol: str, timeframes: List[str]) -> Dict:
        ...
        for tf in timeframes:
            try:
                ...
                bitget_interval = self._map_timeframe_to_bitget(tf)
                if not bitget_interval:
                    logging.warning(f"  ⚠️  No mapping for timeframe: {tf}")
                    continue
                
                # Determine data limit based on timeframe
                if tf in ['2y', '1y', '5M', '4M', '2M', '2w']:
                    # These require special handling or resampling
                    bitget_interval = '1day'  # Use daily and resample
    ...
    def _map_timeframe_to_bitget(self, tf: str) -> Optional[str]:
        ...
        mappings = {
            ...
            '4h': '4h',
            '6h': '6h',
            '8h': '6h',  # Bitget doesn't have 8h, use 6h
            '12h': '12h',
            '1d': '1day',
            '1day': '1day',
            '1w': '1week',
        ...
        # Special handling for macro timeframes
        if tf_lower in ['2y', '1y', '5m', '4m', '2m', '2w']:
            return '1day'  # Use daily data and resample
        
        return mappings.get(tf_lower)
    ...
    def _resample_data(self, df: pd.DataFrame, target_tf: str) -> pd.DataFrame:
        ...
        try:
            ...
            # Resample rules based on target timeframe
            resample_rules = {
                '2y': '2W',
                '1y': '1W',
                '5M': '5D',
        ...
        try:
            ...
            async with BitgetAPI(
                ...
                if symbols:
                    ...
                    # Test klines with valid timeframe
                    ...
                    if klines:
                        ...
                        # Show sample data
                        sample = klines[0] if klines else []
                        if sample:
                            timestamp = datetime.fromtimestamp(int(sample[0])/1000).strftime('%Y-%m-%d %H:%M:%S')
                            open_price = float(sample[1])
                            high_price = float(sample[2])
                            low_price = float(sample[3])
                            close_price = float(sample[4])
                        ...
                        await update.message.reply_text("⚠️ Klines fetch returned no data. Testing with 5min timeframe...")
                        # Try with 5min timeframe
                        klines = await client.get_klines('BTC/USDT', '5min', 5)
                        if klines:
                            await update.message.reply_text(f"✅ 5min klines data successful! Got {len(klines)} candles.")
                        else:
                            await update.message.reply_text("❌ Klines fetch failed with all timeframes.")
                else:
...
async def main():
    ...
    try:
        ...
        async with BitgetAPI(
            ...
            # Test data fetch with valid timeframe
            klines = await client.get_klines('BTC/USDT', '5min', 5)
            if klines:
                ...
                if klines:
                    sample = klines[0]
                    timestamp = datetime.fromtimestamp(int(sample[0])/1000).strftime('%Y-%m-%d %H:%M:%S')
                    logging.info(f"Sample candle: {timestamp} O:{sample[1]} H:{sample[2]} L:{sample[3]} C:{sample[4]}")
            else:
                logging.warning("⚠️ Klines fetch returned no data")
                
                # Try different timeframes
                test_timeframes = ['1h', '4h', '1day', '15min']
                for tf in test_timeframes:
                    klines = await client.get_klines('BTC/USDT', tf, 2)
                    if klines:
                        logging.info(f"✅ {tf} klines successful: {len(klines)} candles")
                        break
    except Exception as e:
