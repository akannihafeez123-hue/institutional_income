        ...
        try:
            ...
            async with self.session.get(
                ...
                if data.get('code') == '00000':  # Bitget success code
                    ...
                    # Bitget V3 returns candles in reverse chronological order
                    # We'll return them as-is and let the caller sort them
                    return klines
                else:
...
class MultiTimeframeAnalyzer:
    ...
    def _create_dataframe(self, klines: List) -> pd.DataFrame:
        ...
        try:
            # Bitget V3 returns 8 columns:
            # [timestamp, open, high, low, close, volume, quote_volume, ctm]
            if not klines:
                return pd.DataFrame()
            ...
            if num_columns == 8:
                columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume', 'ctm']
            elif num_columns == 7:
                columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume']
            elif num_columns == 6:
                columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
            else:
                logging.warning(f"Unexpected kline format: {num_columns} columns")
            ...
            for col in required_cols:
                if col not in df.columns:
                    if col == 'volume' and 'quote_volume' in df.columns:
                        df['volume'] = df['quote_volume'] / df['close']
                    else:
                        logging.error(f"Missing required column: {col}")
                        return pd.DataFrame()
            
            # Sort by timestamp (Bitget returns newest first)
