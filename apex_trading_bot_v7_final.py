"""
═══════════════════════════════════════════════════════════════════════════════
    APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET EDITION
    
    Save as: apex_trading_bot_v7_bitget.py
    ...
    Setup Instructions:
    1. Save as: apex_trading_bot_v7_bitget.py
    ...
    5. Run: python apex_trading_bot_v7_bitget.py
    
    Version: 7.0 Bitget Edition - Fixed
    Date: December 2024
═══════════════════════════════════════════════════════════════════════════════
"""
...
print("=" * 80)
print("       APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET")
...
# Timeframe conversion mappings for Bitget
TF_TO_BITGET = {
    '5m': '5m',
    '15m': '15min',
    '30m': '30min',
...
class BitgetAPI:
    """Production Bitget API client with full authentication"""
    ...
    def _get_headers(self, method: str, endpoint: str, body: dict = None) -> Dict:
        ...
        headers = {
            ...
            'User-Agent': 'APEX-Trading-Bot/7.0'
        }
        
        return headers
    
    async def get_klines(self, symbol: str, interval: str, limit: int = 200) -> List:
        """Fetch candlestick/kline data from Bitget"""
    ...
    async def get_orderbook(self, symbol: str, limit: int = 50) -> Dict:
        """Fetch order book data from Bitget"""
        bitget_symbol = symbol.replace('/', '') + '_UMCBL'
    ...
    async def get_ticker(self, symbol: str) -> Dict:
        """Fetch 24h ticker data from Bitget"""
        bitget_symbol = symbol.replace('/', '') + '_UMCBL'
    ...
    async def get_symbols(self) -> List[str]:
        """Get all tradeable USDT perpetual symbols from Bitget"""
                ...
                if data.get('code') == '00000':
                    ...
                    for instrument in instruments:
                        ...
                        if (symbol_name.endswith('USDT') and 
                            ...
                            instrument.get('onboardDate') and 
...
class MultiTimeframeAnalyzer:
    ...
    async def analyze_all(self, client: BitgetAPI, symbol: str, timeframes: List[str]) -> Dict:
        ...
        # Map of custom timeframes to Bitget intervals
        timeframe_to_bitget = {
            '5m': '5m',
            '15m': '15min',
            '1h': '1H',
        ...
        for tf in timeframes:
            try:
                ...
                if tf in ['2y', '1y', '5M', '4M', '2M', '2w']:
                    ...
                    bitget_interval = timeframe_to_bitget.get(tf, '1H')
                
                # Determine data limit based on timeframe
                if tf in ['2y', '1y']:
                    limit = 1000  # Need more data for yearly analysis
                elif tf in ['5M', '4M', '2M']:
                    limit = 500
                else:
                    limit = 200
...
class TelegramBot:
    """Telegram bot interface - FIXED VERSION"""
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            "🤖 <b>APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET EDITION</b>\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "<b>✅ ACTIVE FEATURES</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "🔬 Quantum Market Analysis\n"
...
"""
    ...
    async def cmd_test(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text("🔄 Testing Bitget connection...")
            ...
            # Test Telegram
            await update.message.reply_text(f"✅ Bitget connection successful! Found {len(symbols)} symbols.")
    ...
    async def cmd_restart(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("🔄 Scanner restarted!")
    ...
    async def run_bot_only(self):
        ...
        try:
            ...
            await self.app.bot.send_message(
                ...
                text=f"""
🤖 <b>APEX SYSTEM V7.0 BITGET EDITION</b>
...
<i>Use /restart to begin scanning markets</i>
""",
                parse_mode='HTML'
            )
    ...
    async def run_with_scanner(self):
        ...
        try:
            ...
            await self.app.bot.send_message(
                chat_id=self.chat_id,
                text=f"""
🚀 <b>APEX SYSTEM V7.0 BITGET EDITION ACTIVATED</b> 🚀
...
async def main():
    ...
    try:
        logging.info("🔗 Testing Bitget connection...")
        async with BitgetAPI(
            os.getenv('BITGET_API_KEY'),
            os.getenv('BITGET_API_SECRET'),
            os.getenv('BITGET_API_PASSPHRASE')
            ...
            symbols = await client.get_symbols()
            logging.info(f"✅ Bitget connected! Found {len(symbols)} symbols")
    except Exception as e:
