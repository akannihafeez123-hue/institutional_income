...
class Analyzer:
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                'regime': {
                    'hurst': self.regime.hurst_exponent(prices[-200:]),
                    'fractal': self.regime.fractal_dimension(prices[-100:]),
                    'wavelet': self.regime.wavelet_analysis(prices[-100:])
                },
                'order_flow': {
                ...
                'forecast': {
                    'kalman': self.forecast.kalman_filter(prices[-50:]),
                    'arima': self.forecast.arima_forecast(prices[-100:]),
                    'spectral': self.forecast.spectral_analysis(prices[-200:])
                },
                'game_theory': {
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {
                # Trend
                'ema_20': float(ema_20.iloc[-1]) if not ema_20.empty else 0,
                ...
                'cci': float(cci.iloc[-1]) if not cci.empty else 0,
                ...
                'roc': float(roc.iloc[-1]) if not roc.empty else 0,
                'willr': float(willr.iloc[-1]) if not willr.empty else -50,
                ...
                # Volume
                'vwap': float(vwap.iloc[-1]) if not vwap.empty else 0,
                'obv': float(obv.iloc[-1]) if not obv.empty else 0,
                'mfi': float(mfi.iloc[-1]) if not mfi.empty else 50,
                'cmf': float(cmf.iloc[-1]) if not cmf.empty else 0,
                
                # Additional
                'psar': float(psar.iloc[-1]) if not psar.empty else 0,
                'kc_middle': float(kc.iloc[-1]) if not kc.empty else 0,
                'supertrend_dir': int(supertrend[-1]) if supertrend else 0,
                'vi_plus': float(vi_plus.iloc[-1]) if not vi_plus.empty else 1,
                'vi_minus': float(vi_minus.iloc[-1]) if not vi_minus.empty else 1
            }
...
class NewsSentiment:
    ...
    async def get_sentiment(self, symbol: str) -> Dict:
        ...
        try:
            ...
            news_samples = [
                f"{base_symbol.upper()} shows strong institutional accumulation",
                f"Market makers positioning for {base_symbol.upper()} breakout",
                f"Technical analysis suggests {base_symbol.upper()} at key level",
                f"Whale activity detected in {base_symbol.upper()}",
                f"{base_symbol.upper()} volatility decreasing ahead of major move"
            ]
    ...
    async def analyze_all(self, client: BybitAPI, symbol: str, timeframes: List[str]) -> Dict:
        ...
        for tf in timeframes:
            try:
                ...
                exchange_tf = TF_TO_EXCHANGE.get(tf, '1h')
