        ...
        async with BybitAPI(os.getenv('BYBIT_API_KEY'), os.getenv('BYBIT_API_SECRET')) as client:
            try:
                ...
                while self.is_running:
                    try:
                        for symbol in symbols[:5]:  # Scan first 5 symbols for testing
...
# ═══════════════════════════════════════════════════════════════════════════
# TELEGRAM BOT - FIXED INTEGRATION
# ═══════════════════════════════════════════════════════════════════════════
...
"""
    ...
    async def start_scanner(self):
        ...
        if not self.scanner:
            ...
            logging.info("✅ Scanner task created")
    
    async def run(self):
        """Run bot - FIXED INTEGRATION"""
        try:
