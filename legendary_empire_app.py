...
- Combines: Legendary Airdrop Empire (scaffold), Dashboard, Config Tester
- Adds: debug_imports runner to force visible import-time tracebacks in CI builds
- Import-safe: no network or side-effecting calls at module import
- Dry-run by default; set ENABLE_LIVE_TRANSACTIONS=true to allow live behavior (use with caution)
"""
...
# -------------------------
# debug_imports utility (prints traceback and exits non-zero on failure)
# Use this in CI/build step to ensure import-time errors are visible in logs.
# Example CI command: python legendary_empire_app.py debug-imports
# -------------------------
...
class LegendaryAirdropEmpire:
    def __init__(self):
        ...
        self.scan_semaphore: Optional[asyncio.Semaphore] = None
    ...
    async def claim_airdrop(self, airdrop: Dict) -> Dict:
        ...
        async with self.claim_semaphore:
            ...
            try:
                ...
                # Live transaction logic intentionally omitted
    ...
    async def auto_withdraw(self):
        if not self.config["auto_withdraw"] or not self._live_transactions_enabled():
            return
        # Live withdraw logic intentionally omitted
...
def main():
    ...
    if args.command == "debug-imports":
        # Run debug imports for a quick visible import-time error
        # Modules to test: name of this file without .py
