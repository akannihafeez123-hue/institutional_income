...
- Merges: bot core (AI error engine + claim scaffolding), dashboard, and config tester
- Safe defaults: dry-run mode by default; no destructive network activity at import time
- Enable live transactions with ENABLE_LIVE_TRANSACTIONS=true (use with great caution)
"""
...
class AIErrorEngine:
    ...
    def should_attempt_claim(self, airdrop: Dict) -> Dict:
        ...
        error_key = f"{airdrop.get('protocol', 'unknown')}_{token_key}"
        err = self.error_patterns.get(error_key)
        if err and err.get("count", 0) > 3:
            if time.time() - err.get("last_attempt", 0) < 3600:
                return {"should_attempt": False, "reason": "Recent repeated failures"}
        if self.confidence.get(error_key, 100) < 20:
...
# -------------------------
# Legendary Empire core
# -------------------------
class LegendaryAirdropEmpire:
    def __init__(self):
        self.rpc_endpoints: List[str] = self._load_rpc_endpoints()
        self.current_rpc_index = 0
        self._clients = None
        self.wallets = []
        self.phantom_address_str = None
    ...
    def _validate_startup_env(self):
        ...
        if not phantom:
            raise RuntimeError("PHANTOM_WALLET_ADDRESS is required")
        self.phantom_address_str = phantom
        ...
        async with self.claim_semaphore:
            ...
            try:
                if not self._live_transactions_enabled():
                    await self.send_telegram_message(f"🧪 Dry-run: would claim {airdrop.get('token')} on {airdrop.get('protocol')}")
                ...
                if resolution.get("should_skip"):
                    self.ai_engine.token_blacklist.add(airdrop.get("mint") or airdrop.get("token") or "unknown")
                    await self.send_telegram_message(f"🤖 AI skipped token {airdrop.get('token')}: {resolution.get('reason')}")
    ...
    async def start(self):
        ...
        self.phantom_address = Pubkey.from_string(self.phantom_address_str)

        print("Legendary Empire starting in dry-run mode" if not self._live_transactions_enabled() else "Legendary Empire starting (LIVE transactions enabled)")
        try:
            cycle = 0
            while True:
                if self.is_paused:
...
class LegendaryDashboard:
    def __init__(self):
        ...
        phantom = os.getenv("PHANTOM_WALLET_ADDRESS", "").strip()
        self.rpc_url = rpc
        self.phantom_address_str = phantom
        self.client = None
        self.wallets = []
...
class ConfigTester:
    ...
    async def test_rpc_connection(self) -> bool:
        ...
        try:
            from solana.rpc.async_api import AsyncClient
            client = AsyncClient(rpc_url, timeout=10)
...
async def run_tests():
    tester = ConfigTester()
    code = await tester.run_all_tests()
    return code


def main():
    parser = argparse.ArgumentParser(prog="legendary_empire_app")
    parser.add_argument("command", choices=["run-bot", "run-dashboard", "test-config", "run-both"], help="command to run")
