"""
═══════════════════════════════════════════════════════════════════════════════
    APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET EDITION (FINAL FIX)
    
    Save as: apex_trading_bot_v7_bitget_final.py
    ...
    Setup Instructions:
    1. Save as: apex_trading_bot_v7_bitget_final.py
    ...
    5. Run: python apex_trading_bot_v7_bitget_final.py
    
    Version: 7.0 Bitget Edition - Final Fix
    Date: December 2024
═══════════════════════════════════════════════════════════════════════════════
"""
...
print("=" * 80)
print("       APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET (FINAL FIX)")
            ...
            async with self.session.get(
                ...
                if data.get('code') == '00000':  # Bitget success code
                    ...
                    if klines:
                        ...
                        # Log first kline structure to see columns
                        if len(klines[0]) > 7:
                            logging.debug(f"First kline has {len(klines[0])} columns: {klines[0]}")
                    return klines[::-1]  # Reverse to get chronological order
                else:
                    error_msg = data.get('msg', 'Unknown error')
                    logging.error(f"Bitget Kline API error: {error_msg} - Interval: {interval} -> {bitget_interval}")
                    return []
        except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════════
# MULTI-TIMEFRAME ANALYZER - FIXED FOR 8 COLUMNS
# ═══════════════════════════════════════════════════════════════════════════

class MultiTimeframeAnalyzer:
    """Analyze all 13 timeframes - FIXED for 8 columns data"""
    
    def __init__(self, client: BitgetAPI):
        self.analyzer = Analyzer()
        self.client = client
    
    async def analyze_all(self, symbol: str, timeframes: List[str]) -> Dict:
        """Analyze all specified timeframes"""
        results = {}
        
        # We'll use only supported timeframes to avoid errors
        supported_tfs = {
            '5m': '5min',
            '15m': '15min',
            '30m': '30min',
        ...
        # Use all supported timeframes for analysis
        for tf in timeframes:
            try:
                if tf not in supported_tfs:
                    continue
                    
                bitget_interval = supported_tfs[tf]
                limit = 200
                
                logging.info(f"  Fetching {symbol} {tf} data (Bitget: {bitget_interval})")
                
                # Fetch klines
                klines = await self.client.get_klines(symbol, bitget_interval, limit)
                if not klines:
                    logging.warning(f"  ⚠️  No kline data for {symbol} {tf}")
                    continue
                
                # Create DataFrame with dynamic column handling for 8 columns
    ...
    def _create_dataframe(self, klines: List) -> pd.DataFrame:
        """Create DataFrame from kline data with dynamic column handling for 8 columns"""
        if not klines:
            return pd.DataFrame()
        
        # Check number of columns
        num_columns = len(klines[0])
        
        # Bitget V3 API returns 8 columns: 
        ...
        if num_columns == 8:
            ...
            logging.error(f"Unexpected number of columns: {num_columns}")
            return pd.DataFrame()
        ...
        numeric_cols = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume']
...
class SignalGenerator:
    ...
    async def generate(self, symbol: str, tf_analyses: Dict) -> Optional[Dict]:
        ...
        if not tf_analyses or len(tf_analyses) < 7:  # Need at least 7 timeframes
            logging.warning(f"Insufficient timeframe analyses: {len(tf_analyses)} < 7")
            return None
        ...
        try:
            ...
            # STRICT: Need 6 out of 8 strategies aligned
            ...
            mtf = MultiTimeframeAnalyzer(None)  # Pass None for client, we don't need it here
            ...
            # STRICT: Need 8 out of 14 indicators aligned
...
class Scanner:
    """Main market scanner with timeframe analysis"""
    ...
    async def start(self):
        ...
        logging.info("=" * 80)
        logging.info("🚀 APEX SCANNER STARTED - PRODUCTION MODE")
        logging.info("📊 Using all available timeframes with 85% thresholds")
        logging.info("=" * 80)
                ...
                while self.is_running:
                    try:
                        for batch_idx in range(total_batches):
                            ...
                            logging.info(f"\n📊 Batch {batch_idx + 1}/{total_batches} - Analyzing {len(batch)} symbols")
                            ...
                            if batch_idx < total_batches - 1:
                                logging.info(f"⏳ Waiting 10 seconds before next batch...")
                                await asyncio.sleep(10)
                        
                        logging.info(f"✓ Full scan cycle completed. Next scan in {self.config.scan_interval}s...\n")
    ...
    async def _scan_symbol(self, client: BitgetAPI, symbol: str) -> Optional[Dict]:
        """Scan single symbol across selected timeframes"""
        try:
            logging.info(f"🔍 Scanning {symbol}...")
            
            # Create MTF analyzer with client
            mtf = MultiTimeframeAnalyzer(client)
            
            # Use all supported timeframes
            supported_timeframes = ['5m', '15m', '30m', '1h', '4h', '6h', '12h', '1d', '1w']
            
            # Analyze all supported timeframes
            tf_analyses = await mtf.analyze_all(symbol, supported_timeframes)
            ...
            logging.info(f"  ℹ️  {analyzed_count}/{len(supported_timeframes)} timeframes analyzed for {symbol}")
            
            # Generate signal with strict requirements
            signal = await self.signal_gen.generate(symbol, tf_analyses)
            
            if signal:
                logging.info(f"  🎯 STRONG SIGNAL: {signal['type']} | Score: {signal['score']*100:.1f}% | Confidence: {signal['confidence']*100:.1f}%")
            else:
                logging.info(f"  ○ No strong signal for {symbol} (85% criteria not met)")
    ...
    async def _send_alert(self, symbol: str, signal: Dict):
        ...
        try:
            ...
            logging.info(f"  📢 HIGH-CONFIDENCE ALERT SENT TO TELEGRAM FOR {symbol}!")
    ...
    def _format_alert(self, symbol: str, signal: Dict) -> str:
        ...
        # Top 6 strategies (since we require 6/8 aligned)
        strategies_text = "\n".join([
            f"  • {name.replace('_', ' ').title()}: {data['score']*100:.0f}% - {', '.join(data['signals'][:2])}"
            for name, data in list(signal['strategies']['results'].items())[:6]
        ])
        
        # Timeframes with status
        aligned_tfs = signal['timeframes']['aligned_timeframes']
        total_tfs = signal['timeframes']['total_analyzed']
        
        tf_text = f"{len(aligned_tfs)}/{total_tfs} timeframes aligned"
        
        # Format prices based on value
        price_format = "${:,.4f}" if signal['entry'] < 10 else "${:,.2f}"
        
        return f"""
{emoji} <b>═══ APEX INSTITUTIONAL SIGNAL ═══</b> {emoji}
...
<i>✅ PRODUCTION MODE: 85% minimum thresholds applied</i>
"""
...
class TelegramBot:
    """Telegram bot interface - Production"""
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            "🤖 <b>APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - PRODUCTION MODE</b>\n\n"
            "✅ 8-Column Bitget V3 API data handling\n"
            "✅ 85% Minimum thresholds applied\n"
            "✅ 6/8 Strategies alignment required\n"
            "✅ 7/13 Timeframes alignment required\n"
...
━━━━━━━━━━━━━━━━━━━━━━
<b>⚙️  PRODUCTION CONFIGURATION</b>
━━━━━━━━━━━━━━━━━━━━━━
• Min Score: 85%
• Min Confidence: 85%
• Min Strategies: 6/8
...
Last Alert: {self.scanner.alert_history[-1]['timestamp'].strftime('%Y-%m-%d %H:%M') if self.scanner and self.scanner.alert_history else 'None'}
"""
    ...
    async def cmd_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        total_alerts = len(self.scanner.alert_history)
        long_signals = sum(1 for alert in self.scanner.alert_history if alert['signal']['type'] == 'LONG')
        short_signals = total_alerts - long_signals
        
        avg_score = np.mean([alert['signal']['score'] for alert in self.scanner.alert_history]) * 100
        avg_confidence = np.mean([alert['signal']['confidence'] for alert in self.scanner.alert_history]) * 100
        
        msg = f"""
<b>📊 APEX SYSTEM STATISTICS</b>

━━━━━━━━━━━━━━━━━━━━━━
<b>📈 SIGNAL DISTRIBUTION</b>
━━━━━━━━━━━━━━━━━━━━━━
Total Signals: {total_alerts}
Long Signals: {long_signals} ({long_signals/total_alerts*100:.1f}%)
Short Signals: {short_signals} ({short_signals/total_alerts*100:.1f}%)
...
"""
        ...
        for alert in list(self.scanner.alert_history)[-5:]:
            msg += f"• {alert['symbol']} - {alert['signal']['type']} - {alert['signal']['score']*100:.1f}% - {alert['timestamp'].strftime('%H:%M')}\n"
    ...
    async def cmd_test(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            async with BitgetAPI(
                ...
                symbols = await client.get_symbols()
                if symbols:
                    await update.message.reply_text(f"✅ Bitget V3 API connected! Found {len(symbols)} USDT symbols.")
                    
                    # Test klines with 1h timeframe
                    klines = await client.get_klines('BTC/USDT', '1h', 5)
                    if klines:
                        await update.message.reply_text(f"✅ Klines data successful! Got {len(klines)} candles with {len(klines[0])} columns.")
                    else:
    ...
    async def cmd_scan(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Scan a single symbol manually"""
        try:
            args = context.args
            if not args:
                await update.message.reply_text("⚠️ Please provide a symbol to scan. Example: /scan BTC/USDT")
                return
            ...
            temp_scanner = Scanner(self.config, self.app.bot)
            ...
            async with BitgetAPI(
                ...
                if signal:
                    message = temp_scanner._format_alert(symbol, signal)
                    await update.message.reply_text(message, parse_mode='HTML')
                else:
                    await update.message.reply_text(f"❌ No strong signal generated for {symbol} (85% criteria not met).")
    ...
    async def cmd_restart(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("🔄 Scanner restarted in PRODUCTION MODE with 85% thresholds!")
    
    async def run(self):
        """Run the Telegram bot and start the scanner"""
        logging.info("🤖 Starting Telegram bot and scanner...")
        ...
        try:
            ...
            await self.app.bot.send_message(
                ...
                text=f"""
🤖 <b>APEX SYSTEM V7.0 BITGET EDITION - PRODUCTION</b>
...
✅ Telegram bot online
✅ 8-Column Bitget V3 API handling
✅ 85% Minimum thresholds applied
✅ Scanner started automatically
✅ Monitoring ALL Bitget USDT symbols

━━━━━━━━━━━━━━━━━━━━━━
<b>⚙️  PRODUCTION CONFIG</b>
━━━━━━━━━━━━━━━━━━━━━━
• Min Score: 85%
• Min Confidence: 85%
• Min Strategies: 6/8
...
<i>High-confidence signals only (85%+ threshold)</i>
""",
                parse_mode='HTML'
            )
...
async def main():
    """Main application entry point - PRODUCTION"""
    
    setup_logging()
    
    print("\n" + "=" * 80)
    print("       APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - PRODUCTION")
    print("          8-Column Data Handling - 85% THRESHOLDS")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        async with BitgetAPI(
            ...
            klines = await client.get_klines('BTC/USDT', '1h', 5)
            if klines:
                logging.info(f"✅ Klines data fetch successful: {len(klines)} candles with {len(klines[0])} columns")
            else:
                logging.warning("⚠️ Klines fetch returned no data")
    except Exception as e:
        logging.error(f"❌ Bitget connection failed: {e}")
        logging.error("Please check your Bitget API credentials")
        return
    ...
    config.min_strategies_aligned = 6  # 6 out of 8 strategies
    config.min_timeframes_aligned = 7  # 7 out of 9 timeframes
    config.min_indicators_aligned = 8  # 8 out of 14 indicators
    config.scan_interval = 60  # 60 seconds between full scans
    ...
    logging.info("  • 7/9 Timeframes Alignment Required")
    logging.info("  • 8/14 Indicators Alignment Required")
    logging.info("  • Scanning ALL Bitget USDT symbols")
    logging.info("=" * 80 + "\n")
    ...
    try:
        # Run bot with scanner
        await bot.run()
        
    except KeyboardInterrupt:
        logging.info("\n" + "=" * 80)
        logging.info("🛑 Production shutdown initiated by user")
        logging.info("=" * 80)
        if bot.scanner:
            bot.scanner.stop()
        await bot.app.stop()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("🛑 APEX Production System stopped by user")
        print("=" * 80)
    except Exception as e:
        print(f"\n❌ Critical error: {e}")
        import traceback
