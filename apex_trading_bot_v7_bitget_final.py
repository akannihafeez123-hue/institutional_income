"""
═══════════════════════════════════════════════════════════════════════════════
    APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET EDITION (FIXED)
...
class ApexConfig:
    ...
    # Scanning settings
    scan_interval: int = 60  # seconds between scans (increased for stability)
            ...
            async with self.session.get(
                ...
                if data.get('code') == '00000':
                    ...
                    if symbols:
                        ...
                        return sorted_symbols[:20]  # Limit to top 20 for performance
                    else:
...
class MultiTimeframeAnalyzer:
    ...
    async def analyze_all(self, symbol: str, timeframes: List[str]) -> Dict:
        ...
        # For testing, let's use only a few timeframes first
        test_tfs = ['5m', '15m', '1h', '4h', '1d', '1w']
        
        for tf in test_tfs:  # Start with only supported timeframes
            try:
                # Get Bitget interval
                bitget_interval = supported_tfs.get(tf, '1h')
                limit = 200
                ...
                # Check number of columns in first kline
                if klines and len(klines[0]) > 7:
                    logging.info(f"  Found {len(klines[0])} columns in kline data")
                
                # Create DataFrame with dynamic column handling
    ...
    def _create_dataframe(self, klines: List) -> pd.DataFrame:
        """Create DataFrame from kline data with dynamic column handling"""
        if not klines:
            return pd.DataFrame()
        ...
        if num_columns == 7:
            ...
            columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume', 'extra']
        else:
            # Default to first 7 columns
            columns = [f'col{i}' for i in range(min(7, num_columns))]
            logging.warning(f"Unexpected number of columns: {num_columns}, using first 7")
        
        # Create DataFrame
        df = pd.DataFrame(klines, columns=columns[:min(7, num_columns)])
...
class SignalGenerator:
    ...
    async def generate(self, symbol: str, tf_analyses: Dict) -> Optional[Dict]:
        ...
        if not tf_analyses or len(tf_analyses) < 3:  # Reduced from 7 for testing
        ...
        try:
            ...
            if aligned_strategies < 4:  # Reduced from 6 for testing
                logging.warning(f"  Insufficient strategies aligned: {aligned_strategies} < 4")
                return None
            
            # Check timeframe alignment
            mtf = MultiTimeframeAnalyzer(None)  # Pass None for client, we don't need it here
            tf_alignment = mtf.check_alignment(tf_analyses, 3)  # Reduced from 7 for testing
            ...
            if not tf_alignment['aligned']:
                logging.warning(f"  Timeframes not aligned: {tf_alignment['bullish_count']}/{tf_alignment['bearish_count']} (need 3)")
                return None
            ...
            if indicators_aligned < 6:  # Reduced from 8 for testing
                logging.warning(f"  Insufficient indicators aligned: {indicators_aligned} < 6")
                return None
            ...
            # Temporarily reduce threshold for testing
            if final_score < 0.70:  # Reduced from 0.85 for testing
                logging.warning(f"  Score below threshold: {final_score:.3f} < 0.70")
                return None
            ...
            if confidence < 0.70:  # Reduced from 0.85 for testing
                logging.warning(f"  Confidence below threshold: {confidence:.3f} < 0.70")
                return None
...
class Scanner:
    ...
    async def start(self):
        ...
        logging.info("=" * 80)
        logging.info("🚀 APEX SCANNER STARTED - TIMEFRAME ANALYSIS")
        logging.info("📊 Using reduced timeframe set for testing")
        logging.info("=" * 80)
        ...
        async with BitgetAPI(
            ...
            try:
                ...
                logging.info(f"📈 Monitoring {len(symbols[:5])} symbols (limited for testing)\n")
    ...
    async def _scan_symbol(self, client: BitgetAPI, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            supported_timeframes = ['5m', '15m', '1h', '4h', '1d', '1w']
            
            # Analyze selected timeframes
            ...
            logging.info(f"  ℹ️  {analyzed_count}/6 timeframes analyzed for {symbol}")
            
            # Generate signal with reduced requirements
    ...
    def _format_alert(self, symbol: str, signal: Dict) -> str:
        ...
        return f"""
{emoji} <b>═══ APEX TEST SIGNAL ═══</b> {emoji}
...
<i>⚠️ TEST MODE: Reduced requirements for testing</i>
"""
...
class TelegramBot:
    """Telegram bot interface - Simplified"""
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            "🤖 <b>APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - TEST MODE</b>\n\n"
            "✅ Data fetching fixed for 8 columns\n"
            "✅ Reduced requirements for testing\n"
            "✅ Testing with 6 timeframes only\n\n"
            "<b>📱 Commands:</b>\n"
...
━━━━━━━━━━━━━━━━━━━━━━
<b>⚙️  TEST CONFIGURATION</b>
━━━━━━━━━━━━━━━━━━━━━━
• Min Score: 70% (reduced)
• Min Confidence: 70% (reduced)
• Min Strategies: 4/8 (reduced)
• Min Timeframes: 3/6 (reduced)
...
"""
    ...
    async def cmd_test(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            async with BitgetAPI(
                ...
                symbols = await client.get_symbols()
                if symbols:
                    await update.message.reply_text(f"✅ Bitget V3 API connected! Found {len(symbols)} symbols.")
                    
                    # Test klines with 1h timeframe
                    klines = await client.get_klines('BTC/USDT', '1h', 5)
                    if klines:
                        await update.message.reply_text(f"✅ Klines data successful! Got {len(klines)} candles.")
                        # Show columns info
                        if klines:
                            await update.message.reply_text(f"📊 First candle has {len(klines[0])} columns")
                    else:
        ...
        try:
            ...
            async with BitgetAPI(
                ...
                if signal:
                    ...
                    await update.message.reply_text(f"❌ No signal generated for {symbol}.")
    ...
    async def cmd_restart(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("🔄 Scanner restarted in TEST MODE!")
    ...
    async def run_bot_only(self):
        ...
        try:
            ...
            await self.app.bot.send_message(
                ...
                text=f"""
🤖 <b>APEX SYSTEM V7.0 BITGET EDITION - TEST MODE</b>
...
✅ Telegram bot online
✅ Data fetching fixed (8 columns handled)
✅ Reduced requirements for testing
...
<i>Use /restart to begin scanning in TEST MODE</i>
""",
                parse_mode='HTML'
            )
...
# ═══════════════════════════════════════════════════════════════════════════
# MAIN ENTRY POINT - FINAL FIX
# ═══════════════════════════════════════════════════════════════════════════

async def main():
    """Main application entry point - FINAL FIX"""
    
    setup_logging()
    
    print("\n" + "=" * 80)
    print("       APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - FINAL FIX")
    print("           8-Column Data Handling - TEST MODE")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        async with BitgetAPI(
            ...
            if klines:
                ...
                # Show column info
                if klines:
                    logging.info(f"📊 First candle has {len(klines[0])} columns")
                    logging.info(f"Sample data structure: {klines[0][:8]}")
            else:
    ...
    # Load configuration
    config = ApexConfig()
    config.scan_interval = 60  # Increase for stability
    
    logging.info("=" * 80)
    logging.info("TEST CONFIGURATION:")
    logging.info("  • Reduced requirements for testing")
    logging.info("  • Using only 6 timeframes: 5m, 15m, 1h, 4h, 1d, 1w")
    logging.info("  • Min Score: 70% (reduced from 85%)")
    logging.info("  • Min Strategies: 4/8 (reduced from 6/8)")
    logging.info("  • Min Timeframes: 3/6 (reduced from 7/13)")
    logging.info("=" * 80 + "\n")
