"""
═══════════════════════════════════════════════════════════════════════════════
    APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET EDITION (FIXED V3)
    ...
    Version: 7.0 Bitget Edition - Final V3 API Fix
    Date: December 2024
═══════════════════════════════════════════════════════════════════════════════
"""
...
print("=" * 80)
print("       APEX INSTITUTIONAL AI TRADING SYSTEM V7.0 - BITGET (FIXED V3)")
...
# ═══════════════════════════════════════════════════════════════════════════
# BITGET API CLIENT - UPDATED FOR V3 API (CORRECT ENDPOINTS)
# ═══════════════════════════════════════════════════════════════════════════

class BitgetAPI:
    """Production Bitget API client with correct V3 API endpoints"""
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 200) -> List:
        ...
        # CORRECT V3 API endpoint for mix market candles
        ...
        try:
            ...
            async with self.session.get(
                ...
                if data.get('code') == '00000':  # Bitget success code
                    ...
                    # Bitget returns candles in format: [timestamp, open, high, low, close, volume, quote_volume]
                    # Return in chronological order (oldest first)
                    return klines[::-1]
                else:
                    logging.error(f"Bitget Kline API error: {data.get('msg', 'Unknown error')}, code: {data.get('code')}")
                    return []
        except Exception as e:
    ...
    async def get_symbols(self) -> List[str]:
        ...
        # CORRECT endpoint for mix contracts
        endpoint = '/api/v2/mix/market/contracts'
        params = {'productType': 'umcbl'}  # lowercase for this endpoint
        ...
        try:
            ...
            async with self.session.get(
                ...
                if data.get('code') == '00000':
                    ...
                    logging.error(f"Bitget Symbol fetch API error: {data.get('msg', 'Unknown error')}, code: {data.get('code')}")
                    return [
                        'BTC/USDT', 'ETH/USDT', 'SOL/USDT', 'BNB/USDT',
                        'XRP/USDT', 'ADA/USDT', 'DOGE/USDT', 'MATIC/USDT'
                    ]
    ...
    async def get_account_info(self) -> Dict:
        ...
        params = {'productType': 'umcbl'}  # lowercase
...
class MultiTimeframeAnalyzer:
    ...
    def _create_dataframe(self, klines: List) -> pd.DataFrame:
        """Create DataFrame from Bitget V3 kline data"""
        try:
            if not klines:
                return pd.DataFrame()
            
            # Bitget V3 returns candles in format: [timestamp, open, high, low, close, volume, quote_volume]
            # Some endpoints may return 8 columns including ctm
            sample = klines[0]
            num_columns = len(sample)
            
            if num_columns >= 7:
                # We need at least 7 columns: timestamp, open, high, low, close, volume, quote_volume
                columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume'][:num_columns]
                if num_columns > 7:
                    columns = columns + [f'extra{i}' for i in range(num_columns - 7)]
            elif num_columns == 6:
                columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
            else:
                logging.error(f"Unexpected kline format: {num_columns} columns, expected at least 6")
                return pd.DataFrame()
            
            df = pd.DataFrame(klines, columns=columns)
            
            # Sort by timestamp (Bitget returns newest first, we need oldest first)
            ...
            # Ensure volume exists - if not, create from quote_volume
            if 'volume' not in df.columns and 'quote_volume' in df.columns and 'close' in df.columns:
                df['volume'] = df['quote_volume'] / df['close']
            ...
            if df.empty:
                logging.warning("DataFrame is empty after processing")
        ...
        for tf in timeframes:
            try:
                ...
                if tf in ['2y', '1y', '5M', '4M', '2M', '2w']:
                    # These are custom timeframes that need resampling
                    # Use 1D data and resample to create these timeframes
                    bitget_interval = '1D'
                ...
                logging.info(f"  Fetching {symbol} {tf} data (Bitget interval: {bitget_interval})")
                ...
                logging.info(f"  Received {len(klines)} candles for {symbol} {tf}")
                
                # Convert to DataFrame
                df = self._create_dataframe(klines)
                if df.empty or len(df) < 50:
                    logging.warning(f"  ⚠️  Insufficient data for {symbol} {tf}: {len(df)} rows after processing")
                    continue
    ...
    def _resample_data(self, df: pd.DataFrame, target_tf: str) -> pd.DataFrame:
        ...
        try:
            ...
            # Resample rules for custom timeframes
            resample_rules = {
                '2y': '730D',  # Approximately 2 years
                '1y': '365D',  # 1 year
                '5M': '150D',  # 5 months
                '4M': '120D',  # 4 months
                '2M': '60D',   # 2 months
            ...
            rule = resample_rules.get(target_tf, '30D')
            ...
            logging.error(f"Resample error for {target_tf}: {e}")
            # Return original data if resampling fails
            return df.reset_index() if 'timestamp' in df.index.names else df
...
class Scanner:
    ...
    async def start(self):
        ...
        async with BitgetAPI(
            ...
            try:
                ...
                logging.info(f"🔧 Testing data fetch for {test_symbol}...")
                test_data = await client.get_klines(test_symbol, '1h', 5)
                if test_data:
                    logging.info(f"✅ Test data fetch successful: {len(test_data)} candles")
                else:
                    logging.warning("⚠️ Test data fetch failed")
                
                while self.is_running:
                    try:
                        # Scan symbols
                        scanned_count = 0
                        for symbol in symbols[:10]:  # Start with fewer symbols for testing
                        ...
                        logging.info(f"✓ Scan cycle completed. Scanned {scanned_count} symbols. Next scan in {self.config.scan_interval}s...\n")
    ...
    async def _scan_symbol(self, client: BitgetAPI, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            logging.info(f"  ✅ {analyzed_count} timeframes analyzed successfully")
...
async def main():
    ...
    try:
        ...
        async with BitgetAPI(
            ...
            # Test data fetch
            test_symbol = symbols[0] if symbols else 'BTC/USDT'
            klines = await client.get_klines(test_symbol, '1h', 5)
            if klines:
                logging.info(f"✅ Klines data fetch successful: {len(klines)} candles for {test_symbol}")
                logging.info(f"Sample candle: {klines[0] if klines else 'No data'}")
            else:
