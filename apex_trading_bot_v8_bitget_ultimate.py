...
═══════════════════════════════════════════════════════════════════════════════
    ...
    2. Create requirements.txt with ONLY LIGHT dependencies:
       pandas==2.0.3
       numpy==1.24.3
       aiohttp==3.9.1
...
# ═══════════════════════════════════════════════════════════════════════════
# COMPLETE DEPENDENCY INSTALLER - ALL PACKAGES AT RUNTIME
# ═══════════════════════════════════════════════════════════════════════════

class CompleteDependencyInstaller:
    """Install ALL dependencies at runtime"""
    
    # List of ALL dependencies to install at runtime
    ALL_PACKAGES = [
    ...
    def install_all_dependencies(cls):
        ...
        for package in cls.ALL_PACKAGES:
        ...
        if failed_packages:
            print(f"   Failed: {len(failed_packages)} packages: {', '.join(failed_packages)}")
        print()
        
        return installed_packages, failed_packages

# Install ALL dependencies
installed, failed = CompleteDependencyInstaller.install_all_dependencies()
...
try:
    ...
    print("  ✓ PyTorch & Transformers imported")
except ImportError as e:
    print(f"  ⚠️ PyTorch/Transformers import warning: {e}")
    torch = None
    nn = None
...
try:
    ...
    print(f"  ⚠️ SciPy import warning: {e}")
    signal = None
    stats = None
...
try:
    ...
    import nltk
    print("  ✓ NLP packages imported")
except ImportError as e:
    print(f"  ⚠️ NLP import warning: {e}")
...
try:
    ...
    print(f"  ⚠️ Advanced analytics import warning: {e}")

# Try to import crypto/finance
try:
    import yfinance as yf
    import optuna
    print("  ✓ Crypto/finance packages imported")
except ImportError as e:
    print(f"  ⚠️ Crypto/finance import warning: {e}")
...
class BitgetAPI:
    ...
    async def get_symbols(self) -> List[str]:
        """Get all tradeable USDT symbols"""
...
class HuggingFaceNewsAnalyzer:
    ...
    def __init__(self, api_token: str = None):
        ...
        try:
            if api_token and 'transformers' in sys.modules:
                try:
...
class ApexConfig:
    ...
    min_strategies_aligned: int = 6
    min_indicators_aligned: int = 8
    min_timeframes_aligned: int = 7
...
class AIStrategySelector:
    ...
    def __init__(self):
        ...
        self.all_strategies = {
            ...
            'fibonacci_vortex': {
                ...
                'description': 'Sacred geometry + vortex convergence',
            ...
            'quantum_entanglement': {
                ...
                'description': 'Quantum probability wave analysis',
            ...
            'dark_pool': {
                ...
                'description': 'Stealth institutional buying detection',
    ...
    def select_best_strategies(self, market_conditions: Dict, user_criteria: Dict = None) -> List[Dict]:
        ...
        top_strategies = []
        for strategy_id, data in sorted_strategies[:3]:
            if data['score'] > 0:
...
class AdvancedSignalGenerator:
    ...
    async def generate_advanced_signal(self, symbol: str, tf_analyses: Dict, 
        ...
        try:
            ...
            strategy_results = {}
            for strategy_info in recommended_strategies[:2]:
...
# ═══════════════════════════════════════════════════════════════════════════
# ULTIMATE SCANNER - SIMPLIFIED VERSION
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# SIMPLE SCANNER WITHOUT TELEGRAM BOT COMPLEXITY
# ═══════════════════════════════════════════════════════════════════════════

class SimpleUltimateScanner:
    """Simple scanner without Telegram bot complexity"""
    ...
    def __init__(self, config: ApexConfig, hf_token: str = None):
        ...
        if token:
            try:
                ...
                logging.warning(f"⚠️ Could not initialize Telegram bot: {e}")
                self.telegram_bot = None
    ...
    async def start_scanning(self):
        ...
        if self.telegram_bot and self.chat_id:
            try:
                await self.telegram_bot.send_message(
                    ...
                    text="🤖 <b>APEX ULTIMATE SCANNER STARTED</b>\n\n✅ 85% Minimum Thresholds\n✅ AI Strategy Selection\n✅ Scanning top 10 symbols",
                    parse_mode='HTML'
                )
            except Exception as e:
        ...
        async with BitgetAPI(
            ...
            try:
                symbols = await client.get_symbols()
                logging.info(f"📈 Found {len(symbols)} Bitget USDT symbols")
                
                # Use first 10 symbols
                test_symbols = symbols[:10]
                logging.info(f"🔍 Monitoring: {', '.join(test_symbols)}")
                ...
                while self.is_running:
                    try:
                        ...
                        logging.info(f"✅ Scan #{scan_count} completed. Next in {self.config.scan_interval}s")
                        await asyncio.sleep(self.config.scan_interval)
    ...
    async def _scan_symbol_ultimate(self, client: BitgetAPI, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if signal:
                ...
                logging.info(f"  ○ No signal (below 85% threshold)")
    ...
    def _calculate_basic_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            bb_upper = bb.bollinger_hband().iloc[-1]
            bb_lower = bb.bollinger_lband().iloc[-1]
            bb_width = (bb_upper - bb_lower) / close.iloc[-1] if close.iloc[-1] > 0 else 0
    ...
    def _calculate_trend(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            logging.error(f"❌ Trend calculation error: {e}")
            return {'strength': 0.5, 'direction': 'neutral', 'slope': 0}
    ...
    def _analyze_structure(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            logging.error(f"❌ Structure analysis error: {e}")
            return {'structure': 'neutral', 'higher_highs': False, 'higher_lows': False}
    ...
    async def _send_ultimate_alert(self, symbol: str, signal: Dict):
        ...
        try:
            ...
            tp_text = "\n".join([
                f"  TP{level['level']}: ${level['price']:,} (+{level['distance_pct']:.1f}%)"
                for level in plan['take_profits'][:2]  # Only show first 2 TP levels
            ])
...
"""
            ...
            # Also log to console
            logging.info(f"  📢 SIGNAL ALERT FOR {symbol}:")
            logging.info(f"     Type: {signal['type']}")
            logging.info(f"     Score: {signal['score']*100:.1f}%")
            logging.info(f"     Entry: ${plan['entry']:,}")
            logging.info(f"     Stop Loss: ${plan['stop_loss']:,}")
...
def setup_logging():
    ...
    file_handler = logging.FileHandler(
        f'logs/apex_{datetime.now():%Y%m%d_%H%M%S}.log'
    )
...
async def main():
    ...
    if missing:
        ...
        logging.error("  export BITGET_API_KEY='your_key'")
        logging.error("  export BITGET_API_SECRET='your_secret'")
        logging.error("  export BITGET_API_PASSPHRASE='your_passphrase'")
        return
    ...
    if not telegram_token or not telegram_chat_id:
        logging.warning("⚠️ Telegram credentials not found. Alerts will only be logged to console.")
    ...
    try:
        ...
        async with BitgetAPI(
            ...
            logging.info(f"📊 Top 10 symbols: {', '.join(top_symbols)}")
    except Exception as e:
        logging.error(f"❌ Bitget connection failed: {e}")
        return
    
    # Test Telegram if credentials provided
    if telegram_token:
        try:
            ...
            logging.warning("⚠️ Continuing without Telegram alerts...")
    
    # Load configuration with 85% thresholds
    config = ApexConfig()
    config.min_signal_threshold = 0.85
    config.min_confidence = 0.85
    config.scan_interval = 60  # 60 seconds between scans
    ...
    logging.info("  • Scan Interval: 60 seconds")
    logging.info("=" * 80 + "\n")
    
    logging.info("🚀 Starting Ultimate Scanner...")
    
    # Create and start scanner
    scanner = SimpleUltimateScanner(config, hf_token)
